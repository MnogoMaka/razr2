#!/usr/bin/env python3
"""
Проверка фотографии камеры на «законное разрытие».

Главная функция ``camera_coordinate_matches_order(image_link, lat, lng, azimuth_deg=None, ...)``
делает следующее и возвращает **словарь Python** (при необходимости сериализуйте в JSON сами):

  1) Прогоняет изображение (URL http(s) или локальный путь) через ансамбль YOLO‑моделей
     из ``YOLO_MODELS_DIR`` (по умолчанию ``result_modelv2``, как в ``processing_job.py``).
  2) Если YOLO‑label != 1 (разрытия нет) — сразу
     ``{"opening": False, "legal": True, "description": "No opening found", ...}``.
  3) Иначе ищет действующий ордер в ``renovation_ii.table_oati_uved_order_raskopki``
     (фильтры: «Виды работ» IS NOT NULL, «Статус» = 'Действует'; колонка ``wkt`` —
     текстовая POLYGON/MULTIPOLYGON, lon lat в WGS84):
       * есть полигон в радиусе ``distance_m`` и (азимут не задан ИЛИ полигон в
         секторе обзора ``azimuth_deg ± fov_deg/2``) → ``legal=True``;
       * есть полигоны в радиусе, но ни один не в секторе → ``legal=False``,
         ``description="Wrong azimuth direction"``;
       * ничего в радиусе → ``legal=False``,
         ``description="There are no orders nearby"``.

Возвращаемый словарь:
  {
    "opening": bool,                   # YOLO нашёл разрытие
    "legal": bool,                     # законно (внутри/в радиусе+азимут или нет разрытия)
    "description": str,                # причина решения
    "image": str,                      # исходная ссылка/путь на фото
    "photo_coordinate": {"lat": ..., "lng": ..., "azimuth_deg": ...?},
    "order_coord": None | GeoJSON,     # подходящий полигон (или ближайший при wrong_azimuth)
    "yolo": {"label": int|None, "votes": [...]},
  }

Логирование (логгер ``camera_order_match``):
  - **INFO** — входные параметры, YOLO‑этапы и голоса моделей, поисковый bbox,
    число кандидатов из STRtree, дуга обзора, дуга полигона, итоговый статус.
  - **DEBUG** — сырой ответ ``STRtree.query``, по каждому кандидату: тип, bounds,
    contains/intersects, расстояние (EPSG:3857), углы до вершин полигона.

.env: DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD, YOLO_MODELS_DIR (необязательно).
"""
